...
public class MemoryMQ<T> implements Destroyable {
    ...
    private void consume(List<T> list) {
        ...
        if (memoryMQConsumer instanceof BatchMemoryMQConsumer) {
            try {
                ((BatchMemoryMQConsumer) memoryMQConsumer).consume(list);
            } catch (Exception e) {
                logger.error("batch consume err, size:{}", list.size(), e);
            }
        } else {
            for (T t : list) {
                try {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("batch consume size:{} use:{}ms", list.size(), (System.currentTimeMillis() - start));
        }
    }
    ...
    public void setMemoryMQConsumer(MemoryMQConsumer<?> memoryMQConsumer) {
        this.memoryMQConsumer = (MemoryMQConsumer<T>) memoryMQConsumer;
    }
     ...
     * @param <T>
     */
    public static interface MemoryMQConsumer<T> {
        /**
         * 消费数据
         * 
         * @param t
     ...
     * @param <T>
     */
    public static interface BatchMemoryMQConsumer<T> extends MemoryMQConsumer<List<T>> {
    }
}
