...
public abstract class LeapArray<T> {
    ...
    public LeapArray(int sampleCount, int intervalInMs) {
        assertTrue(sampleCount > 0, "bucket count is invalid: " + sampleCount);
        assertTrue(intervalInMs > 0, "total time interval of the sliding window should be positive");
        assertTrue(intervalInMs % sampleCount == 0, "time span needs to be evenly divided");
    ...
    public AtomicReferenceArray<WindowWrap<T>> getArray() {
        return array;
    }
     ...
     */
    public int liveTime() {
        return (int) (System.currentTimeMillis() - activeTimestamp);
    }
    
    /**
     * 重置活跃时间
     */
    public void resetActiveTimestamp() {
    ...
    public int survivalTime() {
        return (int) (System.currentTimeMillis() - createTimestamp);
    }
